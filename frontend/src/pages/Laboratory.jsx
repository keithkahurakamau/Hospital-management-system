...
const Laboratory = () => {
    ...
    const callPatient = async (patient) => {
        try {
            ...
            fetchQueue();
        } catch (err) { alert("Failed to initialize sample testing."); }
    };
                        ...
                        queue.map(p => (
                            ...
                            >
                                <div className="flex justify-between items-start mb-2">
                                    <span className={`text-[8px] lg:text-[9px] font-black px-1.5 lg:px-2 py-0.5 lg:py-1 rounded-md uppercase tracking-widest border ${activePatient?.queue_id === p.queue_id ? 'bg-white/10 border-white/20 text-white' : getAcuityStyle(p.acuity_level)}`}>
                                        {p.acuity_level === 1 ? 'Emergency' : p.acuity_level === 2 ? 'Urgent' : 'Standard'}
                                    </span>
                                    <span className={`text-[9px] lg:text-[10px] font-bold flex items-center gap-1 ${activePatient?.queue_id === p.queue_id ? 'text-slate-400' : 'text-slate-400'}`}>
                                        <Clock size={10} className="lg:w-3 lg:h-3"/> {new Date(p.joined_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                                    </span>
                                </div>
                                <h3 className={`font-bold text-xs lg:text-sm uppercase tracking-tight truncate ${activePatient?.queue_id === p.queue_id ? 'text-white' : 'text-slate-800'}`}>
                                    {p.patient_name}
                                </h3>
                                <div className="flex items-center justify-between mt-1.5 lg:mt-2">
                                    <p className={`text-[9px] lg:text-[10px] font-black ${activePatient?.queue_id === p.queue_id ? 'text-slate-500' : 'text-slate-400'}`}>
                                        {p.outpatient_no}
                                    </p>
            ...
            <div className="flex-1 bg-white rounded-[24px] lg:rounded-[32px] border border-slate-200 shadow-sm flex flex-col overflow-hidden relative min-h-[500px] lg:h-[85vh]">
                {!activePatient ? (
                    <div className="h-full flex flex-col items-center justify-center text-slate-300 bg-slate-50/20 py-16 lg:py-0">
                        ...
                        <p className="font-medium text-xs lg:text-sm mt-1.5 lg:mt-2 px-6 text-center">Select a request from the queue to start processing findings.</p>
                    </div>
                ) : (
                    <>
                        <div className="p-5 lg:p-8 border-b border-slate-100 flex justify-between items-center bg-white">
                            <div className="flex items-center gap-3 lg:gap-5">
                                ...
                                <div className="min-w-0">
                                    ...
                                    <p className="text-slate-400 font-bold text-[10px] lg:text-xs uppercase tracking-widest mt-0.5 lg:mt-1">{activePatient.outpatient_no} | {activePatient.sex}</p>
                                </div>
                            </div>
                        </div>

                        <div className="flex-1 p-5 lg:p-8 overflow-y-auto space-y-5 lg:space-y-8 bg-slate-50/30 custom-scrollbar">
                            {/* Doctor's Request Notes */}
                            <div className="bg-blue-50 border border-blue-100 p-4 lg:p-6 rounded-[20px] lg:rounded-[24px] shadow-sm relative overflow-hidden group">
                                <div className="absolute -right-4 -top-4 text-blue-100 group-hover:scale-110 transition-transform duration-500">
                                    <Stethoscope size={80} className="lg:w-[100px] lg:h-[100px]" />
                                </div>
                                <div className="relative z-10">
                                    <p className="text-[9px] lg:text-[10px] font-black text-blue-500 uppercase tracking-widest mb-2 lg:mb-3 flex items-center gap-1.5">
                                        <AlertCircle size={12} className="lg:w-[14px] lg:h-[14px]" /> Incoming Clinical Instructions
                                    </p>
                                    <p className="text-xs lg:text-sm font-bold text-blue-900 leading-relaxed italic pr-4">
                            ...
                            <div className="space-y-3 lg:space-y-4">
                                ...
                                <textarea 
                                    className="w-full p-4 lg:p-6 bg-white border border-slate-200 rounded-[20px] lg:rounded-[28px] text-xs lg:text-sm font-medium leading-relaxed focus:outline-none focus:border-slate-400 transition-all shadow-sm min-h-[250px] lg:min-h-[350px] resize-none placeholder:text-slate-300"
                        ...
                        <div className="p-5 lg:p-6 bg-white border-t border-slate-100 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.05)]">
                            ...
                            >
                                {isSubmitting ? <Activity className="animate-spin" size={14} className="lg:w-4 lg:h-4" /> : <CheckCircle2 size={14} className="lg:w-4 lg:h-4"/>}
